...
internal sealed record ReferenceSearchCommandRequest(
    string Target,
    ...
    IReadOnlyList<string> Fields)
{
    ...
    public static ReferenceSearchCommandRequest Create(
        string target,
        string? solution,
        string? project,
        bool includeTests,
        ...
        IReadOnlyList<string> fields)
    {
        ...
        {
            throw Usage(
                "usage.reference_target_required",
                "The reference target must be a non-blank declaration query or symbol/v2 ID.",
        ...
        {
            throw Usage(
                "usage.reference_limit",
                "The --limit value is invalid for this reference search.",
        ...
        return new ReferenceSearchCommandRequest(
            target,
    ...
    private static string? Optional(string? value, string option)
    {
        ...
        {
            throw Usage(
                "usage.reference_build_selector",
...
internal sealed class ReferenceSearchCommandHandler :
    ICommandHandler<ReferenceSearchCommandRequest>
{
    public async ValueTask<ICommandResult> HandleAsync(
        ReferenceSearchCommandRequest request,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(request);
        var fields = ReferenceFields.Select(request.Fields);
        ...
        if (scope.Selection is null)
        {
            throw new CommandUsageException(
                ...
                "Reference search requires one selected solution or C# project.",
        ...
        try
        {
            result = await new RoslynReferenceSearcher(
                    scope.Traverser,
                    scope.Ownership,
                    scope.Projects)
                .FindAsync(
        ...
        {
            var failurePayload = new ReferenceSearchFailurePayload(
            ...
            return CommandResult<ReferenceSearchFailurePayload>.Failed(
                "search references",
                [new ResultError(
        ...
        return result.Coverage.Level is CoverageLevel.Complete
            ? CommandResult<ReferenceSearchPayload>.Success(
                "search references",
                payload,
                evidence)
            : CommandResult<ReferenceSearchPayload>.Partial(
                "search references",
                payload,
                evidence);
    }
        ...
        return new Evidence(
            ...
            new EvidenceScope(
                ...
                result.TargetResolved
                    ? "evaluated reverse project-graph reference candidates"
                    : "semantic target resolution before reference traversal",
    ...
    private static IEnumerable<IReadOnlyDictionary<string, object?>> Project(
        ...
        OutputFieldSelection<RoslynReferenceMatch> fields,
        CancellationToken cancellationToken)
    {
    ...
    private static string RetrievalCommand(
        ...
        ResolvedSymbolWorkspaceScope scope)
    {
        var command = "dnaxi search references "
    ...
    private static readonly OutputFieldSet<RoslynReferenceMatch>
        ReferenceFields = new(
        [
            ...
            new("implicit", static match => match.IsImplicit),
            new("alias", static match => match.Alias),
            new("candidate_reason", static match => match.CandidateReason),
    ...
    private sealed record ReferenceSearchFailurePayload(
        OperationClassification Classification,
        string Target,
        string TargetStatus,
        int CandidateCount,
