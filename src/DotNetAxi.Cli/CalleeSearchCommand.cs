...
internal sealed record CalleeSearchCommandRequest(
    string Target,
    ...
    IReadOnlyList<string> Fields)
{
    ...
    public static CalleeSearchCommandRequest Create(
        string target,
        string? solution,
        string? project,
        bool includeTests,
        ...
        IReadOnlyList<string> fields)
    {
        ...
        {
            throw Usage(
                "usage.callee_target_required",
                "The callee target must be a non-blank declaration query or symbol/v2 ID.",
        ...
        if (limit < 0 || full && limitSpecified)
        {
            throw Usage(
                "usage.callee_limit",
                "The --limit value is invalid for this callee search.",
        ...
        return new CalleeSearchCommandRequest(
            target,
...
internal sealed class CalleeSearchCommandHandler :
    ICommandHandler<CalleeSearchCommandRequest>
{
    public async ValueTask<ICommandResult> HandleAsync(
        CalleeSearchCommandRequest request,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(request);
        var fields = CalleeFields.Select(request.Fields);
        ...
        if (scope.Selection is null)
        {
            throw new CommandUsageException(
                "usage.callee_workspace_selection_required",
                "Callee search requires one selected solution or C# project.",
        ...
        try
        {
            result = await new RoslynCalleeSearcher(
                    scope.Traverser,
                    scope.Ownership,
                    scope.Projects)
                .FindAsync(
        ...
        {
            var failurePayload = new CalleeSearchFailurePayload(
            ...
            return CommandResult<CalleeSearchFailurePayload>.Failed(
                "search callees",
                [new ResultError(
        ...
        var payload = new CalleeSearchPayload(
            OperationClassification.Executing,
            result.Target,
            result.TargetId!,
        ...
        return result.Coverage.Level is CoverageLevel.Complete
            ? CommandResult<CalleeSearchPayload>.Success(
                "search callees",
                payload,
                evidence)
            : CommandResult<CalleeSearchPayload>.Partial(
                "search callees",
                payload,
                evidence);
    }
        ...
        return new Evidence(
            ...
            new EvidenceScope(
                ...
                result.TargetResolved
                    ? "evaluated target project variants"
                    : "semantic target resolution before callee body analysis",
    ...
    private static IEnumerable<IReadOnlyDictionary<string, object?>> Project(
        ...
        OutputFieldSelection<RoslynCalleeMatch> fields,
        CancellationToken cancellationToken)
    {
    ...
    private static string RetrievalCommand(
        ...
        ResolvedSymbolWorkspaceScope scope)
    {
        var command = "dnaxi search callees "
    ...
    private static readonly OutputFieldSet<RoslynCalleeMatch>
        CalleeFields = new(
        [
            ...
            new("target_identity", static match => match.TargetIdentity, includedByDefault: true),
    ...
    private sealed record CalleeSearchPayload(
    ...
    private sealed record CalleeSearchFailurePayload(
        OperationClassification Classification,
        string Target,
        string TargetStatus,
        int CandidateCount,
