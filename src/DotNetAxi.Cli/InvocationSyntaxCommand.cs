...
internal sealed record InvocationSyntaxCommandRequest(
    string Name,
    ...
    IReadOnlyList<string> Paths)
{
    ...
    public static InvocationSyntaxCommandRequest Create(
        string name,
        bool includeGenerated,
        int limit,
        bool limitSpecified,
        ...
        IReadOnlyList<string> paths)
    {
        ...
        return new InvocationSyntaxCommandRequest(
            name,
            includeGenerated,
            limit,
            limitSpecified,
...
internal sealed class InvocationSyntaxCommandHandler :
    ICommandHandler<InvocationSyntaxCommandRequest>
{
    public async ValueTask<ICommandResult> HandleAsync(
        InvocationSyntaxCommandRequest request,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(request);
        ValidateRequest(request);
        var fields = InvocationFields.Select(request.Fields);
        ...
        var result = await new RoslynSyntaxEngine(new WorkspacePathTraverser())
            .QueryAsync(
                ...
                new InvocationSyntaxQuery(request.Name),
                cancellationToken)
        ...
        var bounded = BoundedCollection<IReadOnlyDictionary<string, object?>>.FromObserved(
            ...
            totalKnown: true,
            retrievalCommand);
        var payload = new InvocationSyntaxPayload(
            bounded.Count,
            bounded.TotalKnown,
            bounded.Total,
            bounded.Omitted,
        ...
        return CommandResult<InvocationSyntaxPayload>.Success(
            "search syntax invocation",
            payload,
            evidence);
    }

    private static void ValidateRequest(InvocationSyntaxCommandRequest request)
    {
        if (string.IsNullOrWhiteSpace(request.Name))
        {
            throw Usage(
                "usage.syntax_invocation_name_required",
                "The invocation name cannot be blank.",
                "Provide a non-blank terminal invocation name with `--name`.");
        }

        if (request.Name.Contains('\0', StringComparison.Ordinal))
        {
            throw Usage(
                "usage.syntax_invocation_name",
                "The invocation name contains an invalid null character.",
                "Provide a C# invocation identifier without null characters.");
        }
        ...
        if (request.Limit < 0 || (request.Full && request.LimitSpecified))
        {
            throw Usage(
                "usage.syntax_limit",
                "The --limit value is invalid for this invocation.",
    ...
    private static string RetrievalCommand(InvocationSyntaxCommandRequest request)
    {
    ...
    private static readonly OutputFieldSet<StructuralCandidate> InvocationFields =
        new(
        [
            ...
            new("construct", static _ => "invocation", includedByDefault: true),
    ...
    private sealed record InvocationSyntaxPayload(
        int Count,
        bool TotalKnown,
        int? Total,
        int? Omitted,
