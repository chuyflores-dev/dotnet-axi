...
public enum CallerSearchScopeMode
{
    Default,
    Complete,
}

public enum CallerSearchVariantStatus
{
    Analyzed,
    Remaining,
    Excluded,
...
public sealed record CallerSearchVariant
{
    internal CallerSearchVariant(
        string project,
        string? configuration,
        string? framework,
        CallerSearchVariantStatus status,
        string? reason,
        string? correction)
    {
        Project = project;
    ...
    public CallerSearchVariantStatus Status { get; }
...
public sealed record RoslynCallerMatch
{
    internal RoslynCallerMatch(
        string id,
...
public sealed class RoslynCallerSearchResult
{
    internal RoslynCallerSearchResult(
        string target,
        string? targetId,
        SemanticTargetResolutionStatus targetStatus,
        string? snapshot,
        CallerSearchScopeMode scopeMode,
        IEnumerable<RoslynCallerMatch>? matches,
        EvidenceCoverage coverage,
        IEnumerable<CallerSearchVariant>? variants,
    ...
    public CallerSearchScopeMode ScopeMode { get; }

    public IReadOnlyList<RoslynCallerMatch> Matches { get; }

    public EvidenceCoverage Coverage { get; }

    public IReadOnlyList<CallerSearchVariant> Variants { get; }
...
/// <summary>
/// Finds exact compiler callers after resolving one target. The evaluated
...
/// </summary>
public sealed class RoslynCallerSearcher
{
    ...
    public ValueTask<RoslynCallerSearchResult> FindAsync(
        string target,
        WorkspaceDiscoveryResult discovery,
        WorkspaceSelection selection,
        WorkspaceTraversalRequest traversal,
        SymbolDeclarationScope? declarationScope = null,
        CallerSearchScopeMode scopeMode = CallerSearchScopeMode.Default,
    ...
    public async ValueTask<RoslynCallerSearchResult> FindAsync(
        string target,
        WorkspaceDiscoveryResult discovery,
        WorkspaceSelection selection,
        WorkspaceTraversalRequest traversal,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!Enum.IsDefined(scopeMode))
        {
            throw new ArgumentOutOfRangeException(
                nameof(scopeMode),
                scopeMode,
                "The caller-search scope mode is not defined.");
        }
    ...
    internal async ValueTask<RoslynCallerSearchResult> FindResolvedAsync(
        string target,
        WorkspaceDiscoveryResult discovery,
        WorkspaceSelection selection,
        WorkspaceTraversalRequest traversal,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var projectScope = scopeMode is CallerSearchScopeMode.Complete
            ? candidateProjects.Complete
            : candidateProjects.Default;
        var report = _coverageReporter.Report(
            graph,
            scopeMode is CallerSearchScopeMode.Complete
        ...
        var matches = new List<RoslynCallerMatch>();
        var variants = new List<CallerSearchVariant>(plans.Count);
        ...
        var partialReasons = PartialReasons(
            ...
            resolution,
            orderedVariants);
        return new RoslynCallerSearchResult(
            target,
    ...
    private static RoslynCallerSearchResult TargetFailure(
        string target,
        CallerSearchScopeMode scopeMode,
        SemanticTargetResolution resolution) =>
        new(
            ...
            resolution.Variants.Select(static variant =>
                new CallerSearchVariant(
                    variant.ProjectPath,
                    variant.Configuration,
                    variant.Framework,
                    CallerSearchVariantStatus.Failed,
                     ...
                     !coveredProjects.Contains(project)))
        {
            ...
            plans.Add(new VariantPlan(
                ...
                new CallerSearchVariant(
                    ...
                    "Repair project evaluation, then retry `dnaxi search callers`.")));
        }
        ...
        ProjectFrameworkCoverageMode frameworkMode)
    {
            ...
            if (projectSelected && frameworkSelected)
            {
                return new VariantPlan(
                    ...
                    new CallerSearchVariant(
                        ...
                        coverage.Configuration,
                        coverage.Framework,
                        CallerSearchVariantStatus.Analyzed,
                        reason: null,
                        correction: null));
            }
        ...
        return new VariantPlan(
            ...
            new CallerSearchVariant(
                ...
                issue?.Correction
                    ?? "Repair the project or framework, then retry the caller search."));
    }
    ...
    private static async ValueTask<IReadOnlyList<RoslynCallerMatch>>
        FindMatchesAsync(
            string workspaceRoot,
            ProjectVariantCoverage coverage,
            Project project,
            ...
            CancellationToken cancellationToken)
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            var callers = await SymbolFinder.FindReferencesAsync(
                    symbol,
                    project.Solution,
                    documents,
                    cancellationToken)
                .ConfigureAwait(false);
            foreach (var location in callers
                         .SelectMany(static reference => reference.Locations)
                         .Where(location =>
                             location.Document.Project.Id == project.Id
                             && location.Location.IsInSource))
            {
                var details = await CallerDetailsAsync(location, symbol, cancellationToken)
                    .ConfigureAwait(false);
                if (details is null)
                {
                    continue;
    ...
    private static AnalyzedVariant Analyzed(
        ProjectVariantCoverage coverage,
        IReadOnlyList<RoslynCallerMatch> matches,
    ...
    private static RoslynCallerMatch Match(
        string workspaceRoot,
        ProjectVariantCoverage coverage,
        string identity,
        ...
        CallerDetails details)
    {
        ...
        return new RoslynCallerMatch(
            ...
            details.ContainingSymbol,
            details.Relationship,
            "semantic",
            details.Confidence);
    }

    private static async ValueTask<CallerDetails?> CallerDetailsAsync(
        ReferenceLocation reference,
        ISymbol target,
        CancellationToken cancellationToken)
    {
        ...
        var relationship = operation switch
        {
            ...
            IObjectCreationOperation => "direct_call",
            IMethodReferenceOperation => "delegate",
            _ when node.AncestorsAndSelf().OfType<InvocationExpressionSyntax>().Any() =>
                IsDispatchTarget(target) ? "possible_dispatch" : "direct_call",
            _ => null,
        };
        ...
        return new CallerDetails(
            containing.GetDocumentationCommentId()
        ...
        int length)
    {
        ...
        Append(hash, "dotnet-axi/caller/v1");
        ...
        return "caller/v1/" + Convert.ToHexStringLower(hash.GetHashAndReset());
    }
    ...
    private static AnalyzedVariant Failed(
        ...
        new(
            new CallerSearchVariant(
                ...
                reason,
                Correction(reason)),
            Array.Empty<RoslynCallerMatch>(),
            "failed:" + reason);

    private static EvidenceCoverage Coverage(
        EvaluatedProjectGraph graph,
        SemanticTargetResolution target,
        IReadOnlyCollection<CallerSearchVariant> variants)
    {
        var analyzed = variants.Count(static variant =>
            variant.Status is CallerSearchVariantStatus.Analyzed);
        ...
        var excluded = variants.Count(static variant =>
            variant.Status is CallerSearchVariantStatus.Excluded);
        ...
        IEnumerable<CallerSearchVariant> variants)
    {
        ...
        reasons.AddRange(variants
            .Where(static variant =>
                variant.Status is not CallerSearchVariantStatus.Analyzed)
    ...
    {
        using var hash = IncrementalHash.CreateHash(HashAlgorithmName.SHA256);
        Append(hash, "dotnet-axi/caller-search-snapshot/v1");
    ...
    {
        using var hash = IncrementalHash.CreateHash(HashAlgorithmName.SHA256);
        Append(hash, "dotnet-axi/caller-semantic-context/v1");
    ...
    private static string Correction(string reason) =>
        ...
        {
            "metadata.missing" =>
                "Run `dnaxi restore`, repair missing callers, then retry the caller search.",
            "project.compilation_errors" =>
                "Fix compiler errors in the reported project and framework, then retry.",
            "project.path_escape" =>
                "Select a project inside the workspace root.",
            _ =>
                "Repair the reported project or framework, then retry the caller search.",
        };
    ...
    private sealed record CallerDetails(
        string ContainingSymbol,
        string Relationship,
        string Confidence);
}
