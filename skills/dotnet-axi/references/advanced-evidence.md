...
9. In 0.5.0, `context symbol` supports only `declaration`, `owner`, `document`, and `outline`. Treat `references`, `callers`, `callees`, `tests`, and other relationship or graph requests as unavailable capability corrections; do not invent commands, sections, or conclusions.
