...
1. Default to one-shot `dnx dnaxi@0.4.0 --verbosity quiet -- <command>`. Keep the exact version pin and do not require a permanent installation.
2. When a controlled harness supplies `DNAXI_LOCAL_FEED`, keep candidate resolution source-pinned with `dnx dnaxi@0.4.0 --source "$DNAXI_LOCAL_FEED" --verbosity quiet -- <command>`.
...
4. Use `dnx dnaxi@0.4.0 --verbosity quiet --` for a passive workspace summary, `dnx dnaxi@0.4.0 --verbosity quiet -- --help` only when command grammar is unknown, and `dnx dnaxi@0.4.0 --verbosity quiet -- --version` when version identity matters.
