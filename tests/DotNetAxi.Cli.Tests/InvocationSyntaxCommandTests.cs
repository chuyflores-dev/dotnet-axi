...
public sealed class InvocationSyntaxCommandTests
{
    [Fact]
    public async Task Invocation_search_handles_supported_shapes_and_false_candidates()
    {
        ...
        await workspace.WriteAsync(
            "Malformed.cs",
            "class Broken { void M() { Target(");

        var result = await workspace.RunAsync(
            "search", "syntax", "invocation", "--name", "Target", "--full");
    ...
    [Fact]
    public async Task Invocation_search_respects_path_and_generated_scope()
    {
        using var workspace = new TestWorkspace();
        ...
        await workspace.WriteAsync("selected/Selected.cs", "class A { void M() { Hit(); } }");
        await workspace.WriteAsync("other/Other.cs", "class B { void M() { Hit(); } }");
        await workspace.WriteAsync("selected/Generated.g.cs", "class G { void M() { Hit(); } }");
    ...
    [Fact]
    public async Task Invocation_search_bounds_output_and_preserves_the_query_in_retrieval()
    {
        using var workspace = new TestWorkspace();
        await workspace.WriteAsync(
            "Calls.cs",
            "class C { void M() { Hit(); Hit(); } }");
    ...
    [Fact]
    public async Task Empty_invocation_search_is_a_successful_structured_result()
    {
        using var workspace = new TestWorkspace();
    ...
    [Fact]
    public async Task Invocation_help_is_passive_and_documents_candidate_semantics()
    {
    ...
    [Theory]
    [InlineData("search", "syntax", "invocation")]
    [InlineData("search", "syntax", "invocation", "--name", "")]
        ...
        params string[] arguments)
    {
        ...
        Assert.Equal(2, exit);
        Assert.Contains("command: search syntax invocation", output.ToString());
