...
public sealed class CommandHostTests
{
    ...
    public void Production_registry_classifies_every_implemented_command()
    {
        ...
        Assert.Collection(host.Operations,
            ...
            operation =>
            {
                Assert.Equal("search callees", operation.Name);
                Assert.Same(
                    OperationPolicy.ExecutingInspection,
                    operation.Policy);
            },
            ...
            operation =>
            {
                Assert.Equal("search derived", operation.Name);
                Assert.Same(
                    OperationPolicy.ExecutingInspection,
                    operation.Policy);
            },
            ...
            operation =>
            {
                Assert.Equal("graph path", operation.Name);
                Assert.Same(
                    OperationPolicy.ExecutingInspection,
                    operation.Policy);
            },
        ...
        Assert.All(host.Operations, operation =>
        {
            if (operation.Name is not "search references"
                ...
                && operation.Name is not "search callees"
